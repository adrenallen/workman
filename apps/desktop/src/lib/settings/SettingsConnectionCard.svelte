...
<section class="connection-card" aria-live="polite">
  ...
  <div>
    ...
    <p>
      ...
      {:else}
        Keep Settings open while gbuild reconnects. Local appearance, terminal, sidebar, hotkey, and opener settings remain available.
      {/if}
    </p>
...
<style>
  ...
  .eyebrow { color: var(--warning); font: 650 7px/1.2 'JetBrains Mono Variable', monospace; letter-spacing: .08em; text-transform: uppercase; }
  h2 { margin: 4px 0 0; color: var(--text); font-size: 15px; }
  p { max-width: 590px; margin: 5px 0 0; color: var(--muted); font-size: 10px; line-height: 1.5; }
  code { display: block; overflow: hidden; max-width: 640px; margin-top: 8px; color: var(--text-soft); font: 8px/1.4 'JetBrains Mono Variable', monospace; text-overflow: ellipsis; white-space: nowrap; }
  button { min-height: 28px; border: 1px solid var(--border-strong); border-radius: 3px; padding: 0 9px; background: var(--surface-raised); color: var(--text-soft); font: 650 8px 'JetBrains Mono Variable', monospace; cursor: pointer; }
