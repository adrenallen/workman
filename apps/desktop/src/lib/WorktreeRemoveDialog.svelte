<script lang="ts">
  ...
  let knownMissingRegistration = $derived(entry?.status === 'missing');
  ...
  $effect(() => {
    if (knownMissingRegistration) deleteFromDisk = false;
  });
</script>
    ...
    <section class="grid min-h-0 content-start gap-3 overflow-y-auto overscroll-contain px-4 py-4">
      ...
      {#if knownMissingRegistration}
        <div class="grid gap-1 rounded border border-warning/40 bg-warning/10 px-3 py-3 text-sm">
          ...
          <span class="text-xs leading-relaxed text-muted-foreground">The folder is missing. Git worktree removal will not run.</span>
        </div>
      {/if}

      <label class="flex items-start gap-2 rounded border border-destructive/40 bg-destructive/5 px-3 py-3 text-sm">
        <Checkbox bind:checked={deleteFromDisk} disabled={knownMissingRegistration} aria-label="Also delete this project from my computer" />
