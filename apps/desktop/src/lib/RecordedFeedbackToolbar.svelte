<script lang="ts">
  ...
  async function selectTool(next: Tool): Promise<void> {
    ...
    error = null;
    try {
      session = await invoke<NativeFeedbackSession>('feedback_set_tool', { tool, color, width });
    } catch (cause) {
...
<div class="toolbar-shell" data-tauri-drag-region>
  ...
  <button class="finish" type="button" disabled={busy} onclick={() => void finish()}>Finish</button>
</div>
...
<style>
  ...
  .recording { display: flex; min-width: 65px; align-items: center; gap: 8px; padding: 0 4px; }
  ...
  .snap-control { display: flex; min-width: 139px; margin-left: auto; }
  ...
  button.snap-main { min-width: 78px; border-radius: 5px 0 0 5px; }
  button.snap-display { min-width: 61px; border-left: 0; border-radius: 0 5px 5px 0; }
  button.cancel-snap { width: 139px; border-color: color-mix(in srgb, var(--destructive) 55%, var(--border)); justify-content: center; background: color-mix(in srgb, var(--destructive) 12%, var(--card)); }
