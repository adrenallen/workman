...
test('new-agent draft keeps template and agent roster choices independent and persistent', async () => {
  ...
  assert.match(source, /model: requestedModel \|\| undefined/);
  assert.match(source, /withAgentLaunchOptions\(/);
  ...
  assert.match(source, /function selectTemplate\(template: AgentTemplate\)[\s\S]*agentTemplateSelectionChange\(selectedTemplate, template\)[\s\S]*templateId: selection\.id,[\s\S]*agentToolId: selection\.agentToolId/);
  assert.match(source, /function selectStandaloneAgent\(tool: AgentTool\)[\s\S]*templateId: null,[\s\S]*agentToolId: tool\.id/);
  assert.match(source, /function selectTemplateAgent\(tool: AgentTool\)[\s\S]*onChange\(\{[\s\S]*agentToolId: tool\.id/);
