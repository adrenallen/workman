//! Harness-free integration test whose executable doubles as a disposable awmd.

use std::{env, path::PathBuf, process::Command, time::Duration};

use awm_core::Store;
use awmd::{DaemonConfig, DaemonServer, Discovery, database_path, discovery_path, probe};
...
fn run_test() {
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        let output = tokio::process::Command::new(env!("CARGO_BIN_EXE_awm"))
            .arg("--data-dir")
            .arg(data_dir.path())
            .arg("--daemon")
