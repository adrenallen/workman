...
use awmd::{DaemonConfig, DaemonServer, Discovery, sync_awm_yml_file, trust_hash_for_process};
...
impl TestDaemon {
    async fn start() -> Self {
        ...
        registry
            ...
            .unwrap();
        sync_awm_yml_file(&mut registry, 1).unwrap();
        let saved = registry
            .list(Some(1))
            .unwrap()
            .into_iter()
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_awm"));
        command
            .arg("--data-dir")
...
async fn all_cli_commands_drive_a_real_daemon() {
    ...
    assert!(codex_setup.contains("[mcp_servers.awm]"));
    ...
    let added = daemon
        ...
        .await;
    assert!(String::from_utf8_lossy(&added.stdout).contains("Added to awm"));
