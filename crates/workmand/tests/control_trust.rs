...
async fn websocket_syncs_reviews_and_trusts_yml_processes() {
    ...
    .await
    .expect("workman.yml watcher did not resync the changed command");
...
#[tokio::test]
async fn websocket_add_command_writes_trusts_and_watches_new_workman_yml() {
    let server = TestServer::start().await;
    ...
    let path = project.join("workman.yml");
    ...
    .await
    .expect("external workman.yml edit did not revoke in-app trust");
