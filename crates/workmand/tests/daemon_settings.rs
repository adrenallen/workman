...
use workmand::{DaemonConfig, DaemonServer, Discovery, discovery_path};
...
async fn settings_report_mcp_setup_data_and_restart_the_isolated_daemon() {
    ...
    assert_eq!(info["build_id"], workmand::BUILD_ID);
    assert_eq!(
        info["control_protocol_version"],
        workmand::CONTROL_PROTOCOL_VERSION
    );
    ...
    assert!(
        claude["fields"][0]["value"]
            .as_str()
            .unwrap()
            .contains("claude mcp add --transport http workman")
    );
    let codex = setups
        .iter()
    ...
    assert_eq!(hello["build_id"], workmand::BUILD_ID);
    assert_eq!(
