//! One-time migration from the pre-rename awm and gbuild application directories.
...
/// Migrate the platform-default pre-rename data and user config on first workman boot.
pub(crate) fn migrate_default_paths_if_needed(data_dir: &Path) -> io::Result<()> {
    let using_default_data =
        env::var_os("WORKMAN_DATA_DIR").is_none() && data_dir == platform_data_dir("workman");
    ...
    if env::var_os(user_config::WORKMAN_CONFIG_ENV).is_none() {
        let destination = user_config::default_user_config_path("workman");
        for app_name in LEGACY_APP_NAMES {
...
/// Copy an awm/gbuild data directory into an absent or empty workman directory.
///
...
pub fn migrate_legacy_data_dir(legacy: &Path, destination: &Path) -> io::Result<bool> {
    ...
    let parent = destination.parent().ok_or_else(|| {
        io::Error::new(
            ...
            "workman data directory must have a parent",
        )
    })?;
...
fn migrated_root_name(name: &OsStr) -> OsString {
    ...
    for legacy_database in LEGACY_DATABASE_FILES {
        if name == legacy_database {
            return OsString::from(DATABASE_FILE);
        }
        if let Some(suffix) = name.strip_prefix(legacy_database) {
...
fn migrate_legacy_user_config(legacy: &Path, destination: &Path) -> io::Result<bool> {
    ...
    let parent = destination.parent().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "workman user config path must have a parent",
        )
    })?;
    fs::create_dir_all(parent)?;
...
mod tests {
    ...
    #[test]
    fn migration_renames_either_legacy_sqlite_and_excludes_discovery() {
        for legacy_database in LEGACY_DATABASE_FILES {
            let root = tempfile::tempdir().unwrap();
            let legacy = root.path().join("legacy");
            let destination = root.path().join("workman");
            fs::create_dir(&legacy).unwrap();
            fs::write(legacy.join(legacy_database), b"database").unwrap();
