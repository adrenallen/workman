...
use crate::{
    LEGACY_AWM_CONFIG_FILE, LEGACY_GBUILD_CONFIG_FILE, ProcessRegistry, RegistryError,
    SharedProcessRegistry, WORKMAN_CONFIG_FILE, is_process_trusted, project_config_path,
    sync_workman_yml, sync_workman_yml_file,
};
    ...
    async fn record_file_event(&mut self, event: Event) {
        ...
        for (project_id, root) in &self.watched_projects {
            if event
                ...
                .any(|path| {
                    path == Path::new(WORKMAN_CONFIG_FILE)
                        || path == Path::new(LEGACY_AWM_CONFIG_FILE)
...
pub(crate) fn sync_project_config(
    ...
    if project_config_path(Path::new(&project.path)).is_some() {
        sync_workman_yml_file(registry, project.id)?;
    }
    Ok(())
}
...
fn sync_project_config_event(
    ...
    if project_config_path(Path::new(&project.path)).is_some() {
        ...
        sync_workman_yml(registry, project.id, "")?;
    }
    Ok(())
}
