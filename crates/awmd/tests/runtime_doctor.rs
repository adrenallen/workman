...
-> Result<(), Box<dyn Error>> {
    ...
    let user_config = temp.path().join("awm-config.yml");
    ...
    let _environment = EnvGuard::set([
        ...
        ("AWM_CONFIG", user_config.clone().into_os_string()),
    ]);
    ...
    assert!(written.contains("{env:AWM_MCP_TOKEN}"));
