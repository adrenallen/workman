...
use awmd::{DaemonConfig, DaemonServer, Discovery};
...
async fn websocket_syncs_reviews_and_trusts_yml_processes() {
    ...
    .await
    .expect("awm.yml watcher did not resync the changed command");
...
#[tokio::test]
async fn websocket_add_command_writes_trusts_and_watches_new_awm_yml() {
    let server = TestServer::start().await;
    ...
    .await
    .expect("external awm.yml edit did not revoke in-app trust");
