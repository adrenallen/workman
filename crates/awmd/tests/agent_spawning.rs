...
async fn wait_for_fake_agent_context(path: &Path) -> Result<(i64, String, String), Box<dyn Error>> {
    ...
    Err("fake agent did not publish its injected awm context".into())
}
...
-> Result<(), Box<dyn Error>> {
    ...
    std::fs::write(
        &fake_agent,
        "#!/bin/sh\n\
         printf '%s\\n%s\\n%s\\n' \"$AWM_PROCESS_ID\" \"$AWM_MCP_TOKEN\" \"$AWM_MCP_URL\" > \"$1\"\n\
         printf 'ready:%s\\n' \"$AWM_PROCESS_ID\"\n\
         IFS= read -r prompt\n\
         printf 'answer:%s\\n' \"$prompt\"\n\
         sleep 30\n",
    ...
    assert!(instructions.contains(&format!("AWM_PROCESS_ID={process_id}")));
    assert!(instructions.contains(&format!("AWM_MCP_URL={endpoint}")));
    assert!(instructions.contains("${AWM_MCP_TOKEN}"));
    assert!(instructions.contains("configure one from AWM_MCP_URL and AWM_MCP_TOKEN"));
    assert!(instructions.contains("Call whoami() through awm first"));
