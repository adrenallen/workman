    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Untrusted(id) => write!(
                formatter,
                "awm.yml process {id} must be trusted before it can start"
            ),
            Self::NotYmlBacked(id) => {
                write!(formatter, "process {id} is not backed by awm.yml")
            }
            Self::TrustHashMismatch(id) => write!(
                formatter,
                "awm.yml process {id} changed since it was reviewed"
            ),
            Self::InvalidWorkingDirectory {
                ...
                formatter,
                "awm.yml process {process_id} has an invalid working directory: {message}"
            ),
...
impl ProcessRegistry {
    ...
    pub fn acknowledge_known_dialog(
        ...
        let event = ProcessEvent {
            ...
            message: format!(
                "awm auto-acknowledged known first-run {} with Enter",
...
mod tests {
    ...
    use awm_core::{AgentTool, ProcessSource, Project, attention::AttentionState};
    ...
    fn status_view_exposes_attention_and_never_calls_permission_idle() {
        ...
        store
            .put_agent_tool(&AgentTool {
                ...
                enabled: true,
                source: awm_core::AgentToolSource::Local,
            })
            .unwrap();
