...
pub fn mcp_connection_info(discovery: &Discovery) -> McpConnectionInfo {
    ...
    let codex_environment = format!(
        "export AWM_MCP_AUTHORIZATION={}",
        shell_quote(&authorization)
    );
    let codex_config = format!(
        "[mcp_servers.awm]\nurl = {endpoint:?}\nenv_http_headers = {{ \"Authorization\" = \"AWM_MCP_AUTHORIZATION\" }}"
    );
    ...
    let setups = vec![
        McpClientSetup {
            ...
            description: "Run once to add awm to Claude Code's local MCP configuration.",
            fields: vec![field(
                "Shell command",
                claude_command,
...
mod tests {
    ...
    fn mcp_connection_info_covers_each_supported_client() {
        ...
        assert_eq!(
            ...
            "claude mcp add --transport http awm http://127.0.0.1:41731/mcp --header 'Authorization: Bearer secret-token'"
        );

        let codex = info.setup(McpClient::Codex).unwrap();
        assert_eq!(
            codex.fields[0].value,
            "export AWM_MCP_AUTHORIZATION='Bearer secret-token'"
        );
        assert_eq!(
        ...
        assert_eq!(gemini["mcpServers"]["awm"]["httpUrl"], info.endpoint);
        assert_eq!(
            gemini["mcpServers"]["awm"]["headers"]["Authorization"],
            "Bearer secret-token"
        );
        ...
        let opencode: serde_json::Value =
            serde_json::from_str(&info.setup(McpClient::Opencode).unwrap().fields[0].value)
                .unwrap();
        assert_eq!(opencode["mcp"]["awm"]["type"], "remote");
        assert_eq!(opencode["mcp"]["awm"]["url"], info.endpoint);
