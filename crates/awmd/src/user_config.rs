...
use awm_core::{AgentTool, AgentToolSource, Store, StoreError};
use serde::{Deserialize, Serialize};

/// Environment variable overriding the platform-specific user config path.
pub const AWM_CONFIG_ENV: &str = "AWM_CONFIG";

/// Filename used beneath the platform-specific `awm` config directory.
pub const USER_CONFIG_FILE: &str = "config.yml";

/// Top-level per-user awm configuration.
...
/// Resolve the user config path, honoring `AWM_CONFIG` before platform defaults.
pub fn user_config_path() -> PathBuf {
    env::var_os(AWM_CONFIG_ENV).map_or_else(|| default_user_config_path("awm"), PathBuf::from)
}
...
#[cfg(test)]
mod tests {
    use awm_core::{AgentTool, AgentToolSource, Store};
