...
use awm_core::{Actor, Process, ProcessId, ProcessStatus, Project, ProjectId};
use axum::{
...
pub fn streamable_http_service(
    ...
    StreamableHttpService<AwmMcp, LocalSessionManager>,
...
struct IdentifySessionArgs {
    /// Stable awm process ID to associate with this MCP session.
...
impl AwmMcp {
    ...
    #[tool(description = "Manually associate this MCP session with an awm process")]
    ...
    #[tool(description = "Show concise awm MCP help, optionally for one topic")]
    async fn help(&self, Parameters(args): Parameters<HelpArgs>) -> CallToolResult {
        let topic = args.topic.as_deref().unwrap_or("setup");
        let text = match topic {
            "setup" => {
                "Connect to /mcp with Streamable HTTP. Daemon-spawned agents send x-awm-mcp-token; external clients use the daemon bearer token then identify_session."
            }
            "identity" => {
...
impl ServerHandler for AwmMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("awm", env!("CARGO_PKG_VERSION")))
            .with_instructions(
                "awm workspace control. Call whoami first; project tools use explicit, selected, then owning-project scope.",
            )
    }
}
