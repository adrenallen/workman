...
impl AwmMcp {
    ...
    #[tool(description = "Preview the complete consent-gated awm MCP config for one agent runtime")]
    ...
    #[tool(
        description = "Optionally spawn one ephemeral agent and verify its awm whoami roundtrip"
    )]
...
pub(crate) fn spawn_registered_agent(
    ...
    let env = BTreeMap::from([(AWM_MCP_URL_ENV.to_owned(), mcp_url.to_owned())]);
...
pub(crate) async fn deep_check_registered_agent(
    ...
    let prompt = "Use only the MCP server named awm. Call whoami once. When it identifies you, print exactly AWM_DEEP_CHECK_OK and exit.";
    ...
    loop {
        ...
        if identified {
            success = true;
            message = if output.contains("AWM_DEEP_CHECK_OK") {
...
fn mcp_launch_args(tool_type: &str, mcp_url: &str) -> Vec<String> {
    match normalize_tool_type(tool_type).as_str() {
        ...
        "codex" => vec![
            ...
            "-c".to_owned(),
            "mcp_servers.awm.env_http_headers={\"x-awm-mcp-token\"=\"AWM_MCP_TOKEN\"}".to_owned(),
        ],
        _ => Vec::new(),
    }
}
...
fn agent_instructions(
    ...
    let client_wiring = match normalize_tool_type(tool_type).as_str() {
        "claude" | "claude_code" => {
            "This Claude launch already has a strict per-launch MCP config for the server named awm."
        }
        "codex" => {
            "This Codex launch already has per-launch mcp_servers.awm URL and header overrides."
        }
        _ => {
            "If this client does not expose an awm connector automatically, configure one from AWM_MCP_URL and AWM_MCP_TOKEN before using coordination tools."
        }
    };
    format!(
        "[awm context] You are awm process ID {process_id} ({process_name}), in project \
         {project_id} ({project_name}, repo {project_path}). awm set \
         AWM_PROCESS_ID={process_id}, AWM_MCP_URL={mcp_url}, and the secret \
         AWM_MCP_TOKEN environment variable. {client_wiring} The connector must use the exact \
         URL in ${{AWM_MCP_URL}} ({mcp_url}) and send the x-awm-mcp-token header from \
         ${{AWM_MCP_TOKEN}}. Use the MCP server named awm, never a globally configured Solo \
...
mod tests {
    ...
    #[test]
    fn claude_launch_uses_only_the_per_process_awm_connector() {
        ...
        assert!(command.contains("x-awm-mcp-token"));
        assert!(command.contains("${AWM_MCP_TOKEN}"));
        assert!(command.ends_with("--model opus"));
    }

    #[test]
    fn codex_launch_overrides_awm_url_and_process_token_header() {
        let command = command_with_mcp_wiring(
        ...
        .unwrap();
        assert!(command.contains("mcp_servers.awm.url="));
        assert!(command.contains("http://127.0.0.1:43124/mcp"));
        assert!(command.contains("mcp_servers.awm.env_http_headers="));
        assert!(command.contains("AWM_MCP_TOKEN"));
    ...
    fn preamble_carries_identity_project_and_mcp_hints_without_the_secret() {
        ...
        assert!(preamble.contains("AWM_PROCESS_ID=41"));
        assert!(preamble.contains("AWM_MCP_URL=http://127.0.0.1:43126/mcp"));
        assert!(preamble.contains("${AWM_MCP_TOKEN}"));
