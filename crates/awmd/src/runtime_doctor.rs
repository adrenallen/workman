...
use awm_core::{AgentTool, AgentToolId, AgentToolSource};
...
const TOKEN_HEADER: &str = "x-awm-mcp-token";
...
async fn check_agent_tool(tool: AgentTool, environment: &DoctorEnvironment) -> AgentToolHealth {
    ...
    AgentToolHealth {
        ...
        configuration_note: if target.automatic_wiring {
            "Per-launch awm MCP wiring is automatic.".to_owned()
        } else {
            "Preview and approve an awm MCP entry for this runtime.".to_owned()
        },
    }
}
...
fn config_preview_in(
    ...
    if target.automatic_wiring {
        return Ok(AgentToolConfigPreview {
            ...
            message: "This runtime receives an isolated awm MCP connector on every launch; no user config change is needed.".to_owned(),
        });
    }
...
fn desired_server(tool_type: &str, mcp_url: &str) -> Value {
    if tool_type == "opencode" {
        json!({
            "type": "remote",
            "url": mcp_url,
            "headers": { (TOKEN_HEADER): "{env:AWM_MCP_TOKEN}" }
        })
    } else {
        json!({
            "httpUrl": mcp_url,
            "headers": { (TOKEN_HEADER): "$AWM_MCP_TOKEN" }
        })
    }
}
...
fn current_server<'a>(root: &'a Map<String, Value>, tool_type: &str) -> Option<&'a Value> {
    if tool_type == "opencode" {
        root.get("mcp")?.get("servers")?.as_object()?.get("awm")
    } else {
        root.get("mcpServers")?.as_object()?.get("awm")
    }
}
...
fn put_server(root: &mut Map<String, Value>, tool_type: &str, server: Value) -> Result<(), String> {
    if tool_type == "opencode" {
        ...
        servers.insert("awm".to_owned(), server);
    } else {
        object_entry(root, "mcpServers")?.insert("awm".to_owned(), server);
    }
    Ok(())
}
...
mod tests {
    ...
    use awm_core::{AgentTool, AgentToolSource};
    ...
    fn opencode_preview_preserves_config_and_write_requires_exact_consent() {
        ...
        assert!(contents.contains("awm-fixture"));
        assert!(contents.contains("{env:AWM_MCP_TOKEN}"));
        assert!(
    ...
    fn command_parser_skips_env_assignments_and_preserves_quoted_paths() {
        ...
        assert_eq!(
            command_executable("AWM=1 codex --full-auto").as_deref(),
            Some("codex")
        );
    }
