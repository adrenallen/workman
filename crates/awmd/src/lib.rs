//! Loopback-only HTTP and WebSocket control server for `awm`.
...
pub use config::{
    ...
    TrustFieldChange, TrustFields, TrustReview, YmlProcess, is_process_trusted, parse_awm_yml,
    project_config_path, sync_awm_yml, sync_awm_yml_file, trust_hash_for_process,
};
pub use lifecycle::{
...
/// The name of the secure daemon discovery file in the awm data directory.
pub const DISCOVERY_FILE: &str = "daemon.json";

/// The SQLite state file stored beside daemon discovery metadata.
pub const DATABASE_FILE: &str = "awm.sqlite3";
...
impl DaemonServer {
    ...
    pub async fn bind(config: DaemonConfig) -> io::Result<Self> {
        ...
        let store =
            awm_core::Store::open(database_path(&config.data_dir)).map_err(registry_io_error)?;
...
struct TerminalSubscription {
    process_id: Option<awm_core::ProcessId>,
    offset: u64,
}
...
/// Resolve the platform data directory, with `AWM_DATA_DIR` as an explicit override.
pub fn default_data_dir() -> PathBuf {
    if let Some(path) = env::var_os("AWM_DATA_DIR") {
        return PathBuf::from(path);
    }
    migration::platform_data_dir("awm")
}
    ...
    loop {
        ...
        if let Some(status) = child.try_wait()? {
            return Err(io::Error::other(format!(
                "awmd exited before becoming ready: {status}"
            )));
        }
        if Instant::now() >= deadline {
            return Err(io::Error::new(
                io::ErrorKind::TimedOut,
                "timed out waiting for awmd discovery",
            ));
        }
...
mod tests {
    ...
    async fn websocket_drives_full_process_lifecycle_and_bulk_commands() {
        ...
        server
            ...
            .await
            .store()
            .put_project(&awm_core::Project {
                id: 1,
                path: "/tmp/awm-control-test".into(),
