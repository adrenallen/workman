...
use gbuild_core::{ProcessId, ProjectId};
...
fn remove_project(params: Value, registry: &mut ProcessRegistry) -> ControlResult {
    ...
    if !params.confirm_remove {
        return Err((
            "confirmation_required",
            "set confirm_remove=true to remove this project from gbuild".to_owned(),
        ));
    }
...
fn store_error(error: gbuild_core::StoreError) -> (&'static str, String) {
