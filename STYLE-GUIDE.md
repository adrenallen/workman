...
- Agent state always uses `AgentStatusIndicator`: gray static circle = idle, spinning green loader = working, blue static dot = needs input, orange stroked clock on a transparent background = waiting/timer, and red exit icon = exited. Exit tooltips distinguish clean exits from crashes.
- Todo state always uses `TodoStatusIndicator` and the shared `--todo-state-*` tokens: a hollow neutral-gray circle means open/unclaimed, an amber circle-dot means claimed or in progress, a red alert means blocked, and a muted gray check means completed. Amber communicates active ownership without borrowing agent-working green or agent-waiting orange; its filled-square glyph separates claimed ownership from the adjacent stroked waiting clock, while the blue needs-input dot stays distinct from both.
...
- Terminal input is sovereign: application shortcuts must not intercept terminal keystrokes except the documented unfocus, process-cycle, and quick-prompt paths.
- Terminal canvas styling is a user-controlled subsystem, independent of the app's light/dark preference. On a stock macOS install, import the default Terminal.app profile once and use its palette, font, size, spacing, cursor, and bold-color behavior; explicit Workman presets and custom choices always win afterward. Graphite (`#202326`) is the fallback when no native profile is readable. Palette hex values belong in the appearance model or imported profile data; terminal chrome outside the canvas still uses interface tokens.
- Keep xterm's native `normal`/`bold` weights, 1.0 line height, zero letter spacing, block cursor, instant scrolling, and minimum contrast ratio unless an imported terminal profile supplies a corresponding value. A retained escape-free screen may bridge first paint only until raw ANSI replay finishes; never leave it masquerading as the styled terminal. Restore the WebGL addon after a hidden webview loses its context so returning to an agent does not change glyph metrics or box drawing.
- Agent PTYs identify as `WezTerm` because current Node/Bun agent CLIs preserve 24-bit color and negotiate the modified-key protocols that Workman's PTY implements under that capability identity. `Apple_Terminal` is intentionally not advertised: Node resolves it to 256 colors before consulting `COLORTERM`, and less-capable identities can suppress Shift+Enter/modified-key negotiation. Native visual parity comes from the imported profile palette and metrics; `TERM=xterm-256color` and `COLORTERM=truecolor` remain the capability baseline.
