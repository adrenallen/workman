...
- Agent PTYs identify as `Apple_Terminal` to match native Codex and Claude behavior while `TERM=xterm-256color` and `COLORTERM=truecolor` continue to advertise color capability. Do not spoof a richer terminal product merely to unlock product-specific probes; keyboard handling belongs to Workman's terminal protocol and key-encoding paths.
